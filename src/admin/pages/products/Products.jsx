...
const BACKEND_URL = import.meta.env.VITE_SITE_URL;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 text-xl font-semibold text-black">
        Loading Products...
      </div>
    );
  }
  ...
  return (
    <div className="w-full text-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">Products</h1>
          <p className="text-gray-500 mt-1 text-sm sm:text-base">Manage all products</p>
        </div>

        {(admin?.role === "admin" || admin?.permissions?.products?.create) && (
          <button
            onClick={() => navigate("/admin/products/add")}
            className="w-full sm:w-auto bg-black text-white px-5 py-3 rounded-lg hover:bg-gray-800 transition text-center text-sm font-medium"
          >
            Add Product
          </button>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-2xl shadow-md p-6">
          ...
          <input
            ...
            className="w-full border border-gray-300 p-4 rounded-xl text-black outline-none focus:ring-2 focus:ring-black"
          />
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedProducts.length > 0 ? (
                paginatedProducts.map((product, pIdx) => (
                  <tr key={`${product._id || product.id}-${pIdx}`} className="hover:bg-gray-50/60 transition">
                    <td className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h2 className="font-semibold text-base text-gray-800 line-clamp-1">{product.title}</h2>
                        </div>
                      </div>
                    </td>
                      ...
                      {(admin?.role === "admin" || admin?.permissions?.products?.edit) ? (
                        <button
                          ...
                          className={`px-4 py-2 rounded-full text-sm font-medium transition ${
                            product.status === "active" ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-red-100 text-red-700 hover:bg-red-200"
                          }`}
                        >
                          {product.status}
                        </button>
            ...
            return pageNumbers.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={`w-10 h-10 rounded-lg border text-sm font-semibold transition-all duration-200 active:scale-95 ${
                    isActive
      ...
      {galleryOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <button onClick={() => setGalleryOpen(false)} className="absolute top-5 right-5 text-white text-4xl focus:outline-none">
            &times;
          </button>

          {galleryImages.length > 1 && (
            <button onClick={prevImage} className="absolute left-5 text-white text-4xl sm:text-5xl select-none focus:outline-none">
              &#10094;
            </button>
          )}

          {/* CHANGED: Wrapped in getImageUrl utility parser */}
          <img src={getImageUrl(galleryImages[currentImage])} alt="" className="max-h-[80vh] max-w-[85vw] object-contain rounded-xl" />

          {galleryImages.length > 1 && (
            <button onClick={nextImage} className="absolute right-5 text-white text-4xl sm:text-5xl select-none focus:outline-none">
              &#10095;
            </button>
          )}
            ...
            {galleryImages.map((img, idx) => (
              <img
                ...
                className={`w-14 h-14 sm:w-16 sm:h-16 object-cover rounded cursor-pointer border-2 shrink-0 transition ${
                  currentImage === idx ? "border-white scale-105" : "border-transparent opacity-70"
                }`}
              />
            ))}
          </div>
