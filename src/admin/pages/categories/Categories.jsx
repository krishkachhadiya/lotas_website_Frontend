...
export default function CategoriesPage() {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 md:p-8 w-full">
                ...
                {(admin?.role === "admin" ||
                    admin?.permissions?.categories?.create) && (
                        <button
                            ...
                            className="w-full sm:w-auto bg-black text-white text-center px-5 py-3 rounded-lg hover:bg-gray-800 transition text-sm md:text-base font-medium"
                        >
                            Add Category
                        </button>
                    )}
                        ...
                        <tbody className="text-sm md:text-base">
                            {paginatedCategories.map((category) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Title */}
                                        <td className="p-4 font-semibold text-gray-800 break-words max-w-[200px]">
                                            {category.title}
                                        </td>

                                        {/* Slug */}
                                        <td className="p-4 text-gray-700 break-all max-w-[200px]">
                                            {category.slug}
                                        </td>
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                {(admin?.role === "admin" ||
                                                    admin?.permissions?.categories?.edit) && (
                                                        <button
                                                            onClick={() =>
                                                                navigate(`/admin/categories/edit/${currentId}`)
                                                            }
                                                            className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md text-xs md:text-sm font-medium transition"
                                                        >
                                                            Edit
                                                        </button>
                                                    )}
                                                ...
                                                {(admin?.role === "admin" ||
                                                    admin?.permissions?.categories?.delete) && (
                                                        <button
                                                            ...
                                                            className="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-md text-xs md:text-sm font-medium transition"
                                                        >
                                                            Delete
                                                        </button>
                                                    )}
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-6 md:mt-8 text-black select-none flex-wrap px-2">

                    {/* Prev Button */}
                    <button
                        disabled={page === 1}
                        onClick={() => setPage(page - 1)}
                        className={`px-3 md:px-4 py-2 rounded-lg border font-medium text-xs md:text-sm transition-all duration-200 ${page === 1
                        ...
                        return pageNumbers.map((item, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-8 h-8 md:w-10 md:h-10 rounded-lg border text-xs md:text-sm font-semibold transition-all duration-200 active:scale-95 ${isActive
                    ...
                    <button
                        ...
                        onClick={() => setPage(page + 1)}
                        className={`px-3 md:px-4 py-2 rounded-lg border font-medium text-xs md:text-sm transition-all duration-200 ${page === totalPages
                                ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
                                : "bg-white text-black border-gray-300 hover:bg-gray-50 active:scale-95"
                            }`}
                    >
