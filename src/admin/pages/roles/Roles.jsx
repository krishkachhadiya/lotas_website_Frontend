  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8 text-black">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">Roles</h1>
          <p className="text-gray-500 mt-2">Manage system roles & fine-grained permissions</p>
        </div>

        <button
          onClick={() => navigate("/admin/roles/add")}
          className="bg-black text-white px-5 py-3 rounded-xl hover:bg-gray-800 transition font-medium self-start sm:self-auto shadow-sm"
        >
          Add Role
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1.5 w-full sm:w-auto">
          <label className="text-xs font-bold uppercase tracking-wider text-gray-400">Selected Active Role</label>
          <select
            value={selectedRole}
            onChange={(e) => setSelectedRole(e.target.value)}
            className="border bg-white px-4 py-2.5 rounded-xl outline-none min-w-[260px] font-medium border-gray-300 focus:ring-2 focus:ring-black"
          >
            {roles.map((role) => {
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto sm:mt-auto">
          <button
            onClick={() => navigate(`/admin/roles/edit/${selectedRole}`)}
            className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition shadow-sm"
          >
            Edit Role Name
          </button>
          <button
            onClick={() => handleDelete(selectedRole)}
            className="flex-1 sm:flex-initial bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition shadow-sm"
          >
            Delete Role
          </button>
        </div>
          ...
          <table className="w-full min-w-[600px] border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedModules.map((module) => {
                return (
                  <tr key={module} className="hover:bg-gray-50/70 transition">
                    
                    {/* MODULE TITLE */}
                    <td className="p-5 font-semibold capitalize text-gray-800 align-middle">
                      {module}
                    </td>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(() => {
                          ...
                          if (!modulePerms || (!modulePerms.create && !modulePerms.edit && !modulePerms.delete)) {
                            return <span className="text-gray-400 text-sm italic">No Permissions Assigned</span>;
                          }
                          ...
                          return (
                            <>
                              ...
                              {modulePerms.edit && (
                                <span className="bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">
                                  Edit
                                </span>
                              )}
                              {modulePerms.delete && (
                                <span className="bg-red-50 text-red-700 border border-red-200 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">
                                  Delete
                                </span>
                              )}
                            </>
